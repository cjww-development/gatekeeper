...
import dev.cjww.mongo.responses.{MongoSuccessCreate, MongoSuccessUpdate}
  ...
  "findUser" should {
    "return a User" when {
      ...
      "a user matches on more than one field" in {
        val query = mongoAnd(mongoEqual("userName", testUser.userName), mongoEqual("digitalContact.email.address", testUser.digitalContact.email.address))
    ...
    "return None" when {
      ...
      "a user matches on one field" in {
        val query = mongoAnd(mongoEqual("userName", testUser.userName), mongoEqual("digitalContact.email.address", "invalid@email.com"))
        awaitAndAssert(testUserStore.findUser(query)) {
          _ mustBe None
        }
      }
  ...
  "projectValue" should {
    ...
    "return None" when {
      "the document doesn't exist" in {
        awaitAndAssert(testUserStore.projectValue("userName", "invalidUserName", "digitalContact")) {
          _ mustBe Map()
        }
      }
