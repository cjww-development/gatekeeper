...
trait ScopeController extends BaseController {
  ...
  val scopesService: ScopeService

  def getValidScopes: Action[AnyContent] = Action {
    val scopes = scopesService.getValidScopes
    if(scopes.reads.nonEmpty || scopes.writes.nonEmpty) {
